...
const addArticle = async () => {
    ...
    await storeArticle.addArticle(formData, setErrors, processing)
    router.push({ name: 'admin.article.index' })
    ...
    <AuthenticatedLayout title="Create Article">
        <v-container>
            <v-card>
                ...
                <label for="theme" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Title</label>
                <input type="text" v-model="title"
                    ...
                    required />
                <label for="theme" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Content</label>
                <QuillEditor v-model:content="content" contentType="html"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    required />
                <button class="mt-4 flex gap-3 px-3 py-1 items-center justify-center bg-secondary-3 rounded text-white">
                    <p>Submit</p>
