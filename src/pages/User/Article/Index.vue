...
const getstatusClass = status => {
    if (status === 'publish') {
        return {
            container:
                'bg-secondary-3 text-secondary-3 px-2 text-sm font-bold rounded-full inline-flex items-center bg-opacity-30',
            circle: 'fa-duotone fa-solid mr-1 fa-circle-check',
        }
    } else if (status === 'draft') {
        return {
            container:
                'bg-blue-500 text-blue-500 px-2 text-sm inline-flex font-bold rounded-full items-center bg-opacity-30',
            circle: 'mr-1 fa-solid fa-circle-exclamation',
        }
    } else if (status === 'future') {
        return {
            container:
                'bg-orange-500 text-orange-500 px-2 text-sm inline-flex font-bold rounded-full items-center bg-opacity-30',
...
const Update = async item => {
    await router.push({
        ...
        params: { id: item.id, idwp: id.value },
    })
}
