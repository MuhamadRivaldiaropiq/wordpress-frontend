...
const fetchData = async () => {
    ...
    selectedTags.value = article.value?.tags?.map(tagId => {
        return tags.value.find(tag => tag.id === tagId) || { id: tagId, name: `Tag ID ${tagId}` }
    })
    SelectedCategory.value = article.value?.categories?.map(categoryId => {
        return Categorys.value.find(category => category.id === categoryId) || { id: categoryId, name: `Tak Berkategori` }
    })
}
...
const addArticleDraft = async () => {
    ...
    await storeArticle.PublishArticles(formData, setErrors, processing, domain, token)
...
const UpdateArticle = async () => {
    ...
    console.log(id);
    try {
        await storeArticle.UpdateArticles(formData, setErrors, processing, domain, token, id)
        toast.add({ severity: 'success', summary: 'Success', detail: 'Article Berhasil Diupdate', life: 2000 });
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Article Gagal Diupdate!', life: 3000 });
...
const formattedDate = computed({
  ...
  set(value) {
    ...
    article.value.date = new Date(`${datePart}T${timePart}:00`).toISOString();
  }
});
const previewItem = () => {
...
<template>
    <loading-overlay :active="isloading" loader="dots" color="#06D001" :is-full-page="false" />
                    ...
                    <div class="w-[20rem]">
                        <div class="bg-white shadow-xl rounded">
                            <div class="p-3">
                                <label>Tag</label>
                                <hr class="mb-3 mt-1">
                                <multiselect v-model="selectedTags" :options="tags" :multiple="true"
                                    :close-on-select="false" :clear-on-select="false" placeholder="Pilih tag"
                                    label="name" :taggable="true" @tag="addtags" track-by="name"></multiselect>
                            </div>
                        </div>
                        <div class="bg-white shadow-xl mt-5 rounded">
                            <div class="p-3">
                                ...
                                <hr class="mb-3 mt-1">
                                <multiselect v-model="SelectedCategory" :options="Categorys" :multiple="true"
                                    :close-on-select="false" :clear-on-select="false" placeholder="Pilih Category"
                                    label="name" :taggable="true" @tag="AddCategory" track-by="name"></multiselect>
                            </div>
                        </div>
                        <div class="bg-white shadow-xl mt-5 rounded">
                            <div class="p-3">
                                ...
                                <div class="p-2">
                                    <div class="flex items-center">
                                        <div class="flex-1">
                                            <button v-if="article.status == 'draft'"
                                                class="bg-typography-4 text-sm rounded p-2 border border-secondary-3">Simpan
                                                Draf</button>
                                        </div>
                                        <div class="flex-1 text-right">
                                            <button @click="previewItem"
                                                class="bg-typography-4 text-sm rounded p-2 border border-secondary-3">Pratinjau</button>
                                        </div>
                                    </div>

                                    <div class="mt-2 text-base">
                                        <div>Status : <span class="font-semibold">{{ article.status }}</span>   <span class="text-sm cursor-pointer text-secondary-3 underline" @click="DropdownStatus"
                                                v-if="isDropdownStatus == false">Edit</span></div>
                                        <div v-if="isDropdownStatus" class="flex">
                                            <select v-model="SelectedStatus" class="w-full rounded-lg">
                                                ...
                                                <option v-if="article.status == 'publish' || article.status == 'future' " value="draft">Draf</option>
                                            </select>
                                            <p class="text-sm mt-1 cursor-pointer text-red-500 ml-2 flex items-center" @click="NoDropdownStatus"><i class="fa-solid fa-circle-xmark"></i></p>
                                        </div>
                                    </div>
                                    <div class="mt-2 text-base">
                                        <!-- <span v-if="article.status == 'draft'">segera</span> -->
                                        <div>{{ TextDate }}  <span class="font-semibold">{{ formattedDate }} </span> <span class="text-sm cursor-pointer text-secondary-3 underline"
                                                @click="DropdownDate" v-if="isDropdownDate == false">Edit</span>
                                        </div>
                                        <div v-if="isDropdownDate">
                                            <div class="flex">
                                                <input type="datetime-local" v-model="DateArticle" class="w-full rounded-lg">
                                                <p class="text-sm mt-1 cursor-pointer text-red-500 ml-2 flex items-center" @click="NoDropdownDate"><i class="fa-solid fa-circle-xmark"></i></p>
                                            </div>
                                    ...
                                    <div class="flex justify-end mt-3">
                                        <button @click="UpdateArticle" type="button"
                                            class="text-sm bg-secondary-3 p-2 text-typography-1 rounded">{{ ButtonLabel }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
