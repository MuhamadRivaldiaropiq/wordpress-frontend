      ...
      <div class="bg-white shadow-xl mt-5 rounded">
                            <div class="p-3">
                                ...
                                <div class="flex justify-between">
                                    <button class="bg-typography-4 p-2 rounded border border-secondary-3 text-sm"
                                        @click.prevent="handleSaveDraft">Simpan Draf</button>
                                    <button
                                        class="bg-typography-4 p-2 rounded border border-secondary-3 text-sm">Pratinjau</button>
                                </div>
                                <div class="text-base p-3">
                                    <div>Status : <span class="font-semibold">Draft</span> <span
                                            class="text-sm text-secondary-3 underline cursor-pointer"
                                            v-if="isDropdownStatus == false" @click="DropdownStatus">Edit</span> </div>
                                    <div v-if="isDropdownStatus" class="flex mb-3">
                                        ...
                                        <div class="text-sm mt-1 cursor-pointer text-red-500 ml-2 flex items-center"
                                            @click="NoDropdownStatus"><i class="fa-solid fa-circle-xmark"></i></div>
                                    </div>
                                    <div>Terbitkan <span class="font-semibold">Segera</span> <span
                                            class="text-sm text-secondary-3 underline cursor-pointer"
                                            v-if="isDropdownJadwal == false" @click="DropdownJadwal">Edit</span></div>
                                    <div class="flex" v-if="isDropdownJadwal">
                                        <input type="datetime-local" class="rounded-lg w-full" v-model="DateArticle">
                                        <div class="text-sm mt-1 cursor-pointer text-red-500 ml-2 flex items-center"
                                            @click="NoDropdownJadwal"><i class="fa-solid fa-circle-xmark"></i></div>
                                    </div>
                                </div>
                                <hr class="mb-3 mt-1">
                                <div class="flex justify-end">
                                    <button class="bg-secondary-3 rounded p-2 text-typography-1" type="button"
                                        @click="handleSubmit">{{
                                            ButtonLabel }}</button>
                                </div>
                            </div>
